...
function setupSamplerSlot(i, recordBtn, playBtn, countdown) {

    recordBtn.onclick = () => {
        const slot = samplerSlots[i];

        if (!slot.isRecording) {
            slot.element.classList.add("countdown-state");
            let t = 3;
            ...
            const timer = setInterval(() => {
                ...
                if (t <= 0) {
                    clearInterval(timer);
                    countdown.innerText = "";
                    slot.element.classList.remove("countdown-state");
                    slot.element.classList.add("recording");
                    slot.events = [];
            ...
            slot.element.classList.remove("recording");

            if (slot.events.length > 0) {
                slot.element.classList.add("has-recording");
            }
            keyDownTimes = {};
        }
    };
...
function startSlotPlayback(i) {
    const slot = samplerSlots[i];
    slot.element.classList.add("playing");
    if (!slot.events.length) return;

    // ⛔ prevent double-play instantly
    if (slot.playInterval) clearInterval(slot.playInterval);

    const loopLength =
        Math.max(...slot.events.map(e => e.start + e.duration));

    const startTime = performance.now() / 1000;

    // reset trigger state per loop cycle
    slot.playInterval = setInterval(() => {

        const now = performance.now() / 1000;
        const t = (now - startTime) % loopLength;

        slot.events.forEach(e => {

            if (t >= e.start && t < e.start + 0.03) {
...
function stopSlotPlayback(i) {
    ...
    slot.isPlaying = false;
    slot.element.classList.remove("playing");
}
...
const samplerSlots = Array(4).fill(null).map(() => ({
    events: [],
    isRecording: false,
    isPlaying: false,
    playInterval: null,
...
const samplerGrid = document.getElementById("samplerGrid");

for (let i = 0; i < 4; i++) {

    const slot = document.createElement("div");
    slot.className = "sampler-slot";
    ...
    samplerGrid.appendChild(slot);

    setupSamplerSlot(i, recordBtn, playBtn, countdown);
    samplerSlots[i].element = slot;
}
...
document.addEventListener("keydown", (e) => {
    ...
    if (recordingSlot !== null) {
        keyDownTimes[e.key] = performance.now() / 1000;
    }
});
...
document.addEventListener("keyup", (e) => {
    ...
    data.element?.classList.remove("active");
     // 🎯 RECORDING (KEYUP)
    if (recordingSlot !== null && keyDownTimes[e.key] != null) {

        const slot = samplerSlots[recordingSlot];

        slot.events.push({
            key: e.key,
            freq: data.freq,
            start: keyDownTimes[e.key] - recordStartTime,
            duration: (performance.now() / 1000) - keyDownTimes[e.key]
        });
